...
import { SAMPLE_MCQ_QUESTIONS, MCQQuestion } from "@/lib/data";

// Expand questions to 50 by rotating
function generateQuestions(base: MCQQuestion[], count = 50): MCQQuestion[] {
  const result: MCQQuestion[] = [];
  for (let i = 0; i < count; i++) {
    result.push({ ...base[i % base.length], id: `q${i + 1}` });
  }
  return result;
}
...
function TimerRing({ seconds, totalSeconds, isPhysics }: { seconds: number; totalSeconds: number; isPhysics: boolean }) {
    ...
    <div className="relative w-20 h-20 flex items-center justify-center">
      <svg width="80" height="80" className="progress-ring">
        ...
        <circle
          ...
          strokeLinecap="round"
          className={isDanger ? "timer-danger" : ""}
...
export default function MCQPage() {
  ...
  const [questions] = useState(() => generateQuestions(SAMPLE_MCQ_QUESTIONS));
  ...
  const examStarted = phase === "exam";

  const isPhysicsMath = id?.includes("physics") || id?.includes("maths") || id?.includes("chemistry") || id?.includes("biology");
  ...
  }, [current, questions.length, questionTime, submitExam]);
    ...
    const handleVisibility = () => {
      if (document.hidden) {
        setViolations((v) => {
          const newV = v + 1;
          if (newV === 1) {
            setWarning("⚠️ Tab switch detected! First warning.");
            setFocusScore((f) => Math.max(0, f - 15));
          } else if (newV === 2) {
            setWarning("⚠️ SECOND violation! -60 seconds penalty applied.");
            setTimeLeft((t) => Math.max(0, t - 60));
            setFocusScore((f) => Math.max(0, f - 25));
          } else {
            setWarning("🚫 Third violation! Auto-submitting exam...");
  ...
  // Disable right-click, copy-paste, screenshots in exam
  useEffect(() => {
    if (!examStarted) return;
    ...
    const noCopy = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && ["c", "v", "a", "p", "s"].includes(e.key.toLowerCase())) {
        e.preventDefault();
        setWarning("⚠️ Keyboard shortcuts disabled during exam");
  ...
  const weakTopics = [...new Set(
    questions
      .filter((_, i) => answers[i] !== null && answers[i] !== questions[i].correctAnswer)
      .map((q) => q.topic)
  )].slice(0, 3);
      ...
      <div className="min-h-screen grid-bg flex items-center justify-center p-4"
        ...
        <div className="glass-card max-w-2xl w-full p-8" style={{ border: "1px solid rgba(124,58,237,0.4)" }}>
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-black mb-2 gradient-text">MCQ Assessment</h1>
          ...
          <div className="mb-8 p-4 rounded-xl" style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)" }}>
            <h3 className="font-bold text-sm mb-3 text-red-400">🔒 Exam Integrity Rules</h3>
            <div className="space-y-1.5 text-xs" style={{ color: "var(--text-secondary)" }}>
              {[
                "Tab switching will trigger violations",
                "3 violations = automatic exam submission",
                "Right-click and copy-paste are disabled",
                "Exam timer auto-advances if you don't answer",
                "Your focus score is tracked throughout",
              ].map((r, i) => (
          ...
          <div className="flex gap-4">
            ...
            <button
              onClick={() => { setPhase("exam"); setTimeLeft(questionTime); }}
  ...
  if (phase === "exam") {
    ...
    return (
      <div className="exam-overlay" style={{ background: "var(--background)", color: "var(--foreground)" }}
        onContextMenu={(e) => e.preventDefault()}>
        {/* Warning banner */}
        {warning && (
          <div
            className="fixed top-0 left-0 right-0 z-[9999] p-4 text-center font-bold text-sm animate-slide-up"
            style={{ background: violations >= 3 ? "#EF4444" : "#D97706", color: "white" }}
          >
            {warning}
          </div>
        )}
        ...
        {/* Header */}
        <div
          className="sticky top-0 z-50 p-4 flex items-center justify-between border-b"
        ...
        >
          ...
          <div className="flex-1 max-w-md">
            <div className="flex justify-between text-xs mb-1.5" style={{ color: "var(--text-muted)" }}>
              ...
              <span>
                {score} correct · {current - score} wrong
              </span>
            </div>
          ...
          {/* Timer */}
          <TimerRing seconds={timeLeft} totalSeconds={questionTime} isPhysics={isPhysicsMath} />

          {/* Focus + violations */}
          <div className="text-right">
            <div className="text-xs mb-1" style={{ color: "var(--text-muted)" }}>Focus Score</div>
            <div className="font-black text-lg" style={{ color: focusScore >= 80 ? "#10B981" : focusScore >= 50 ? "#F59E0B" : "#EF4444" }}>
              {focusScore}%
            </div>
            <div className="text-xs" style={{ color: violations === 0 ? "#10B981" : "#EF4444" }}>
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          ...
          <div className="flex justify-between items-center mt-6">
            ...
            <div className="text-xs text-center" style={{ color: "var(--text-muted)" }}>
              Answer to auto-advance · Or wait for timer
            </div>
            <button onClick={submitExam} className="px-4 py-2 rounded-xl text-sm font-bold transition-all"
        ...
        <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
          ...
          <div className="flex gap-1 flex-wrap">
            {questions.map((_, i) => (
              <button
                ...
                style={{
                  background: i === reviewIndex ? "rgba(124,58,237,0.5)" :
                    answers[i] === questions[i].correctAnswer ? "rgba(16,185,129,0.3)" :
                    answers[i] !== null ? "rgba(239,68,68,0.3)" : "rgba(255,255,255,0.05)",
                  color: i === reviewIndex ? "#A78BFA" :
                    answers[i] === questions[i].correctAnswer ? "#10B981" :
                    answers[i] !== null ? "#EF4444" : "var(--text-muted)",
                }}
              >
                {i + 1}
              </button>
          ...
          <div className="glass-card p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="px-2 py-0.5 rounded-full text-xs" style={{ background: "rgba(124,58,237,0.2)", color: "#A78BFA" }}>
                {q.topic}
              </span>
            </div>
            ...
            <div className="space-y-2 mb-5">
              {q.options.map((opt, i) => (
                <div
                  ...
                  style={{
                    background: i === q.correctAnswer ? "rgba(16,185,129,0.15)" :
                      i === userAnswer && !isCorrect ? "rgba(239,68,68,0.15)" :
                ...
                >
                  <span
                    ...
                    style={{
                      background: i === q.correctAnswer ? "#10B981" : i === userAnswer && !isCorrect ? "#EF4444" : "rgba(255,255,255,0.1)",
                      color: "white",
                    }}
                  >
                  ...
                  {i === userAnswer && i !== q.correctAnswer && (
                    <span className="ml-auto text-xs" style={{ color: "#EF4444" }}>Your answer</span>
                  )}
                  {i === q.correctAnswer && (
                    <span className="ml-auto text-xs" style={{ color: "#10B981" }}>Correct</span>
                  )}
                </div>
              ))}
            </div>
            ...
            <div className="p-4 rounded-xl" style={{ background: "rgba(124,58,237,0.1)", border: "1px solid rgba(124,58,237,0.2)" }}>
              ...
              <div className="mt-2 text-xs" style={{ color: "var(--text-muted)" }}>Concept: {q.topic} · Type: {q.type}</div>
            </div>
          </div>

          <div className="flex gap-3">
            <button
              onClick={() => setReviewIndex((r) => Math.max(0, r - 1))}
    ...
    <div className="min-h-screen grid-bg flex items-center justify-center p-4"
      ...
      <div className="max-w-2xl w-full space-y-6">
        ...
        <div
          ...
          style={{ border: `1px solid ${passed ? "rgba(16,185,129,0.4)" : "rgba(239,68,68,0.4)"}` }}
        >
          <div className="text-6xl mb-4 animate-bounce-in">{passed ? "🏆" : "📚"}</div>
          ...
          <div className="text-left space-y-3 mb-6">
            <div className="text-sm font-bold">Performance Breakdown</div>
            ...
            <div className="flex justify-between text-xs" style={{ color: "var(--text-muted)" }}>
              ...
              <span>Avg: {Math.round(totalTime / questions.length)}s/question</span>
            </div>
          </div>
            ...
            <div className="p-4 rounded-xl text-left mb-6" style={{ background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.2)" }}>
              ...
              <div className="flex flex-wrap gap-2">
                {weakTopics.map((topic, i) => (
                  <span key={i} className="px-3 py-1 rounded-full text-xs" style={{ background: "rgba(245,158,11,0.2)", color: "#F59E0B" }}>
                    {topic}
                  </span>
                ))}
              </div>
          ...
          <div className="p-4 rounded-xl text-left" style={{ background: "rgba(124,58,237,0.1)", border: "1px solid rgba(124,58,237,0.2)" }}>
            ...
            <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
              {passed
                ? `Excellent performance! You're ready for the next chapter. Focus on strengthening ${weakTopics[0] || "application questions"} for exam excellence.`
                : `You need more practice. Revisit ${weakTopics.join(", ") || "the chapter"} and attempt this test again. Suggested: spend 30 more minutes on theory.`}
            </p>
          </div>
        </div>

        <div className="flex gap-4">
          <button onClick={() => { setPhase("review"); setReviewIndex(0); }} className="flex-1 btn-secondary py-3 rounded-xl">
            📋 Review Answers
          </button>
          <Link href="/" className="flex-1 btn-secondary py-3 rounded-xl text-center">
            🏠 Home
          </Link>
          <button
            onClick={() => {
